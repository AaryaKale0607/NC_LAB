...
|------|---------|---------|
| Plan file | `PLAN-NNN_<short-slug>.md` | `PLAN-001_unit_tests.md` |
| Report file | `REPORT-NNN_<short-slug>.md` | `REPORT-001_unit_tests.md` |
| Slug | lowercase, underscores, ≤ 5 words | `add_operator_overload` |
...
Group by logical unit (e.g., "Unit tests", "Operator overloading").
...
1. **Be concrete, not vague.** Write "Add a divide-by-zero check in `divide()` that returns myComplex(NaN, NaN)" — not "Improve robustness."

2. **Reference files by relative path** from project root (e.g., `src/myComplex.cpp`, `include/myComplex.hpp`).
...
5. **No orphan acronyms.** First use must expand: "Google Test (gtest)", "Code Coverage (gcov)".
...
### Good Plan Title
✅ `PLAN-001_add_unit_test_suite_with_google_test`

### Bad Plan Title
❌ `PLAN-001_testing`

### Good Goal
✅ "Achieve >90% branch coverage on all `myComplex` operations as measured by `gcov`"

### Bad Goal
❌ "Improve test coverage"

### Good Task
✅ "Install Google Test framework and write 5 test cases for `add()` method"

### Bad Task
❌ "Write tests" (too vague, no timeframe)

---

For more guidance, see examples in `AGENT_MD/examples/` (if available in your project).
